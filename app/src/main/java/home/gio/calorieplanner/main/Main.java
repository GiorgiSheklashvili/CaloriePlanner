        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    ...
                    Elements hrefs = subLinks.select("a[href]");
                    int categoriesIndex = 0;
//                    for (int subMenuIndex = 0; subMenuIndex < hrefs.size(); subMenuIndex++) {
//
//                        if (subMenuIndex == 12 || subMenuIndex == 17 || subMenuIndex == 30 || subMenuIndex == 39 || subMenuIndex == 45 || subMenuIndex == 47 || subMenuIndex == 49 || subMenuIndex == 57 || subMenuIndex == 60 || subMenuIndex == 63) {
//                            categoriesIndex++;
//                        }
                    StringBuilder link = new StringBuilder(baseURL + "category.aspx?id=127");
//                        String link = "http://goodwilldelivery.ge/category.aspx?id=107";
                    ...
                    for (int i = 1; i <= pagesQuantity; i++) {
                        ...
                        for (int j = 0; j < links.size(); j++) {//getting picture and name of product
                            tempProduct.setCategory(categories.get(0).text());//category name
                            tempProduct.setSubMenu(hrefs.get(1).text());//submenu name
                    ...
                    Log.d("gamoagwia", "link.size foridan");
...
//                        databaseReference.child("Goodwill").setValue(product);
//                    }
                } catch (Exception ex) {
...
//        }
    ...
    private String getCalories(String details) {
        ...
        if (index == -1)
            return "0";
        Pattern pattern = Pattern.compile("\\d+");
        Matcher matcher = pattern.matcher(details.substring(index, index + 35));
        matcher.find();
        return matcher.group();
    }
